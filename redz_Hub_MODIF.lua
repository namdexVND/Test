...
-- 🟢 Cải tiến Drag & Drop (Chống xung đột click)
local UserInputService = game:GetService("UserInputService")
local dragging = false
local dragStart = nil
local startPos = nil
local moved = false -- Biến kiểm tra xem có di chuyển chuột hay không
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
        ...
        moved = true -- Đánh dấu là đã di chuyển (đang kéo)
    end
end)
...
-- Sự kiện Click (Chỉ thực hiện nếu KHÔNG di chuyển nút)
ImageButton.MouseButton1Up:Connect(function()
    if not moved then -- Nếu không di chuyển thì mới Toggle
        Window:Minimize()
    end
end)
