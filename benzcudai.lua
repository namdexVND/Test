...
-- ==========================================
-- LOAD NEWUILIB FROM GITHUB
-- ==========================================
local function loadNewuiLib()
    local success, response = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/namdexVND/Test/refs/heads/main/NewuiLib.txt")
    end)
    ...
    if success and response then
        ...
        if libFunc then
            ...
            warn("❌ Không thể load NewuiLib từ link")
            return nil
        end
    else
        warn("❌ Lỗi tải NewuiLib: " .. tostring(response))
        return nil
    end
end

-- Load thư viện
local NewuiLib = loadNewuiLib()
if not NewuiLib then
    warn("❌ NewuiLib không thể tải, script không thể chạy!")
    return
end

print("✅ NewuiLib tải thành công!")
...
-- ==========================================
-- NEWUILIB UI WRAPPER - COMPATIBLE LAYER
-- ==========================================
local UIWindow, WindowData = NewuiLib:AddWindow("Benz Script - Blox Fruit", {
...
-- Create all tabs
local tabNames = {"Thông Tin", "Cày Cấp", "Sự Kiện", "Lấy & Nâng Cấp Vật Phẩm", "Cài Đặt", "Webhook", "Chỉ Số", "Người Chơi", "Dịch Chuyển", "Giả Mạo", "Trái Ác Quỷ", "Đột Kích", "Nâng Cấp Chủng Tộc", "Cửa Hàng", "Khác"}
local tabKeys = {"Info", "Main", "Sea", "Item", "Setting", "Status", "Stats", "Player", "Teleport", "Visual", "Fruit", "Raid", "Race", "Shop", "Misc"}

local Tabs = {}
for i, name in ipairs(tabNames) do
    local key = tabKeys[i]
    local tabData, tabUI = WindowData:AddTab(name)
    Tabs[key] = tabData
end

-- Compatibility wrapper for old API
local function wrapNewuiTab(tabData)
    local wrapped = {}
    local sections = {}
    
    function wrapped:AddSection(name)
        -- NewuiLib doesn't have explicit sections, we use labels as section headers
        if name and name ~= "" then
            tabData:AddLabel(name)
        end
        return wrapped
    end
    
    function wrapped:AddToggle(id, setting)
        local title = setting.Title or setting["Title"] or id
        local default = setting.Default or setting["Default"] or false
        local callback = setting.Callback or setting["Callback"]
        
        local switchData, switchUI = tabData:AddSwitch(title, function(value)
            if callback then
        ...
        -- Add OnChanged method for compatibility
        local proxy = {}
        setmetatable(proxy, {
            __index = function(_, k)
                ...
                elseif k == "SetValue" or k == "SetStage" then
                    return function(_, v)
                        if switchData.Set then
                            switchData:Set(v)
                        end
                    end
                elseif k == "GetValue" then
                    return function(_)
                        return switchUI.Text == utf8.char(10003)
                    end
                end
                return nil
            end
    ...
    function wrapped:AddDropdown(id, setting)
        local title = setting.Title or setting["Title"] or id
        local values = setting.Values or setting["Values"] or {}
        local default = setting.Default or setting["Default"] or 1
        local callback = setting.Callback or setting["Callback"]
        
        local dropdownData, dropdownUI = tabData:AddDropdown(title, function(value)
            if callback then
                pcall(callback, value)
            end
        end)
        ...
        setmetatable(proxy, {
            __index = function(_, k)
                if k == "OnChanged" then
                    return function(_, fn)
                        if fn then pcall(fn, values[default] or values[1]) end
                        return proxy
                    end
                elseif k == "SetValue" then
                    return function(_, v)
                        -- Update dropdown value
                        dropdownUI.Text = "      [ " .. tostring(v) .. " ]"
                    end
                elseif k == "GetValue" then
                    return function(_)
    ...
    function wrapped:AddSlider(id, setting)
        ...
        local default = setting.Default or min_val
        local callback = setting.Callback or setting["Callback"]
        
        local sliderData, sliderUI = tabData:AddSlider(title, function(value)
            if callback then
                pcall(callback, value)
            end
        end, {
        ...
        setmetatable(proxy, {
            __index = function(_, k)
                if k == "OnChanged" then
                    return function(_, fn)
                        if fn then pcall(fn, default) end
                        return proxy
                    end
                elseif k == "SetValue" then
                    return function(_, v)
    ...
    function wrapped:AddButton(setting, cb)
        local title = setting.Title or setting["Title"] or "Button"
        local callback = setting.Callback or cb or function() end
        
        tabData:AddButton(title, callback)
    ...
    function wrapped:AddInput(id, setting)
        ...
        tabData:AddTextBox(title, function(value)
            if callback then
                pcall(callback, value)
            end
        end, {clear = true})
...
-- Wrap all tabs with compatibility layer
for key, tab in pairs(Tabs) do
    Tabs[key] = wrapNewuiTab(tab)
end
...
-- CONTINUE WITH CORE FUNCTIONALITY CODE...
-- [All the core functionality from the original script follows here]
-- [This includes all farming, leveling, skills, etc.]

print("✅ Benz Script with NewuiLib loaded successfully!")
-- Anti AFK
game:GetService("Players").LocalPlayer.Idled:connect(function()
    game:GetService("VirtualUser"):Button2Down(Vector2.new(0, 0), workspace.CurrentCamera.CFrame)
    wait()
...
Tabs.Info:AddButton({
    ["Title"] = "benz Script | Community",
    ["Description"] = "Discord",
    ["Callback"] = function()
        setclipboard(tostring("https://discord.gg/Vz28hAJAVb"))
    end
})
Tabs.Info:AddButton({
    ["Title"] = "Youtube",
    ["Description"] = "youtube",
    ["Callback"] = function()
        setclipboard(tostring("https://youtube.com/@benzbandaredz?si=HGHUQuGLCsRfCxgT"))
    end
})
Tabs.Info:AddButton({
    ["Title"] = "benz redz",
    ...
    ["Callback"] = function()
        setclipboard(tostring("https://www.facebook.com/share/1E4KRcvwy6/"))
    end
})
