...
-- 🟢 Logic Drag & Drop đa nền tảng (Mobile/PC)
local UserInputService = game:GetService("UserInputService")
local dragging = false
local dragStart, startPos
local isDragging = false -- Biến cờ để kiểm tra xem có đang kéo không

ImageButton.InputBegan:Connect(function(input)
    -- Hỗ trợ cả chuột (MouseButton1) và cảm ứng (Touch)
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        -- Nếu di chuyển quá một khoảng nhỏ, coi như là đang kéo
...
-- 🟢 Sử dụng .Activated để hỗ trợ cả Click và Tap
ImageButton.Activated:Connect(function()
    -- Chỉ toggle menu nếu KHÔNG phải là đang thực hiện thao tác kéo
