...
-- ============================================
-- 🔴 FLUENT WINDOW & TOGGLE BUTTON - FIXED
-- ============================================
...
-- ✅ NÚT TOGGLE HOẠT ĐỘNG 100% - KHÔNG BỊ LỖI
local WindowFolder = nil
local isMenuOpen = true

-- Hàm tìm Window GUI
local function GetWindowFolder()
	task.wait(0.3)
	for _, obj in pairs(game.CoreGui:GetChildren()) do
		if obj:IsA("ScreenGui") and obj.Name ~= "ToggleButtonGui" then
			if #obj:GetChildren() > 3 then
				return obj
			end
		end
	end
...
ToggleFrame.Parent = ToggleButtonGui
ToggleFrame.BackgroundTransparency = 1
ToggleFrame.Size = UDim2.new(0, 50, 0, 50)
ToggleFrame.Position = UDim2.new(0.01, 0, 0.08, 0)
...
ToggleButton.Image = "http://www.roblox.com/asset/?id=126677255840605"
...
ToggleButton.MouseButton1Click:Connect(function()
	...
	-- Tìm window nếu chưa có
	if not WindowFolder then
		WindowFolder = GetWindowFolder()
	end
	
	-- Toggle
	if WindowFolder then
		WindowFolder.Visible = isMenuOpen
...
-- Hover effect
local function UpdateHoverColor()
...
-- Giám sát và update window folder
task.spawn(function()
	while true do
		task.wait(1)
		if not WindowFolder then
...
print("✅ VND Hub Loaded Successfully!")
print("💡 Click nút góc trái để bật/tắt menu")
